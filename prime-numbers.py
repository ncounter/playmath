...
def isPrime(n):
   ...
   # let's just do the skipping only after the first tens of numbers
   if n > 10:
   ...
   # we would have an even number, and even means it can be divided always also by 2,
   # so it cannot be a prime number at all.
...
def main():
   ...
   # step of 2 starting from the first odd value to skip all even values
   for n in range(min_limit, limit, 2):
      if isPrime(n):
        primes.append(n)
